...
function decodePem(pem, encryptType) {
  ...
  if (encryptType === ENCRYPT_TYPE.ECDSA) {
    ...
    if (pem.startsWith(PRIVATE_KEY_PREFIX_SM)) {
      // -----BEGIN PRIVATE KEY-----
      privateKey = pem.substring(
        PRIVATE_KEY_PREFIX_SM.length,
        PRIVATE_KEY_PREFIX_SM.length + 64
      );
    } else {
...
class Configuration {
  constructor(configFilePath, addUser) {
    if (!configFilePath) {
    ...
    try {
      ...
      if (addUser) {
        config.accounts[addUser.accountName] = {
          type: addUser.type,
          value: addUser.value,
        };
      }
  ...
  _parseEncryptType(config) {
    if (!config.encryptType) {
      ...
      if (typeof encryptType !== "string") {
        throw new ConfigurationError(
          "invalid type of `encryptType` property, `string` expected"
        );
      } else {
        if (encryptType === "ECDSA") {
          ...
          throw new ConfigurationError(
            "invalid value of `encryptType` property, expect `ECDSA` or `SM_CRYPTO`"
          );
        }
      }
    }
      ...
      for (let nodeIndex in nodes) {
        ...
        for (let childProperty of ["ip", "port"]) {
          if (!node[childProperty] || typeof node[childProperty] !== "string") {
            throw new ConfigurationError(
              `invalid ${childProperty} property in \`nodes\` at position ${
                parseInt(nodeIndex) + 1
              }`
            );
          }
        }
  ...
  _parseGroupID(config) {
    ...
    if (config.groupID < 1 || config.groupID > 32767) {
      throw new ConfigurationError(
        "invalid `groupID` property, `groupID` should be within the scope of [1, 32767]"
      );
    }
  ...
  _parseChainID(config) {
    ...
    if (config.chainID < 0 || config.chainID > Number.MAX_SAFE_INTEGER) {
      throw new ConfigurationError(
        `invalid \`chainID\` property, \`chainID\` should be within the scope of [0, ${Number.MAX_SAFE_INTEGER}]`
      );
    }
  ...
  _parseTimeout(config) {
    ...
    if (config.timeout < 0 || config.timeout > Number.MAX_SAFE_INTEGER) {
      throw new ConfigurationError(
        `invalid \`timeout\` property, \`timeout\` should be within the scope of [0, ${Number.MAX_SAFE_INTEGER}]`
      );
    }
  ...
  _parsePrivateKey(privateKey) {
    if (typeof privateKey !== "object") {
      ...
      if (
        !privateKey.type ||
        !["pem", "pemStr", "ecrandom", "p12"].includes(privateKey.type)
      ) {
      ...
      switch (privateKey.type) {
        ...
        case "pemStr": {
          let encodedPem = privateKey.value;
          let decodedPem = pemFile.decode(encodedPem).toString("hex");
